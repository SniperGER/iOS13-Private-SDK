//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Dec  5 2019 09:59:31).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class NSMutableDictionary, NSNumber;

@interface PRSModelManager : NSObject
{
    BOOL _disablePolicy;
    NSNumber *_modelVersion;
    NSNumber *_shadowVersion;
    NSMutableDictionary *_shadowModels;
    NSMutableDictionary *_models;
    NSUInteger _activeCount;
}

+ (void)loadModelWithURL:(id)arg1 type:(NSUInteger)arg2 directivesPath:(id)arg3 intoModelDict:(id)arg4 error:(id )arg5;
+ (id)directivesFromFilePath:(id)arg1;
+ (BOOL)loadModelsWithDirectory:(id)arg1 intoModelDict:(id)arg2;
+ (void)pathsFor:(NSUInteger)arg1 withParentPath:(id)arg2 modelPath:(id )arg3 directivesPath:(id )arg4;
+ (void)cannedInfoForType:(NSUInteger)arg1 directivesPath:(id )arg2 modelName:(id )arg3;
+ (id)sharedModelManager;
@property(nonatomic) NSUInteger activeCount; // @synthesize activeCount=_activeCount;
@property(retain, nonatomic) NSMutableDictionary *models; // @synthesize models=_models;
@property(retain, nonatomic) NSMutableDictionary *shadowModels; // @synthesize shadowModels=_shadowModels;
@property(retain, nonatomic) NSNumber *shadowVersion; // @synthesize shadowVersion=_shadowVersion;
@property(retain, nonatomic) NSNumber *modelVersion; // @synthesize modelVersion=_modelVersion;
@property(nonatomic) BOOL disablePolicy; // @synthesize disablePolicy=_disablePolicy;
// - (void).cxx_destruct;
- (double)testL2WithData:(id)arg1 experimental:(BOOL)arg2;
- (float )computeScoresForFeatures:(id)arg1 withBundleFeatures:(id)arg2 serverBundleFeatures:(id)arg3 usingModelContext:(id)arg4 qos:(unsigned int)arg5 shouldCancel:(BOOL )arg6 filterBundle:(id)arg7;
- (float )computeL2ScoresForVectors:(id)arg1 secondVector:(id)arg2 withServerFeatures:(id)arg3 withBundleFeatures:(id)arg4 experimentalWeight1:(double)arg5 experimentalWeight2:(double)arg6 forShadow:(BOOL)arg7 shouldCancel:(BOOL )arg8 clientBundle:(id)arg9 hasMail:(BOOL)arg10;
- (void)updateModelsAccordingToHierarchy;
- (void)updateModelsAccordingToHierarchy:(id)arg1 updateEnabled:(BOOL)arg2 disablePendingUpdates:(BOOL)arg3;
- (id)loadIfNecessaryModelOfResourceType:(NSUInteger)arg1 nowTime:(double)arg2 intoModelDict:(id)arg3 currentVersion:(id)arg4;
- (void)loadCannedModels;
- (void)loadCannedModelWithType:(NSUInteger)arg1 error:(id )arg2;
- (void)triggerUpdate;
- (void)deactivate;
- (void)activate;
- (id)getL2ModelVersionForClientBundle:(id)arg1;
- (void)dealloc;
- (id)init;

@end

