//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@protocol OS_os_log;

@interface NADelegateMethodLogSettings : NSObject
{
    unsigned char _logType;
    NSObject<OS_os_log> *_log;
    id /* block */ _argumentFormatter;
    id /* block */ _logger;
}

@property(copy, nonatomic) id /* block */ logger; // @synthesize logger=_logger;
@property(readonly, copy, nonatomic) id /* block */ argumentFormatter; // @synthesize argumentFormatter=_argumentFormatter;
@property(readonly, nonatomic) unsigned char logType; // @synthesize logType=_logType;
@property(readonly, nonatomic) NSObject<OS_os_log> *log; // @synthesize log=_log;
- (id)formattedDescriptionForArgument:(id)arg1;
- (id)initWithLog:(id)arg1 logType:(unsigned char)arg2 argumentFormatter:(id /* block */)arg3;
- (id)init;

@end

