//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Dec  5 2019 09:59:31).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <PhotosUICore/PXGCGImageTextureProvider.h>

@interface PXGStringTextureProvider : PXGCGImageTextureProvider
{
}

- (void)_requestTextureForString:(id)arg1 withAttributes:(id)arg2 targetSize:(CGSize)arg3 userInterfaceDirection:(NSUInteger)arg4 verticalAlignment:(long long)arg5 screenScale:(double)arg6 textureRequestID:(int)arg7;
// - (NSRange *)requestTexturesForSpritesInRange:(struct _PXGSpriteIndexRange)arg1 geometries:(CDStruct_3ab912e1 )arg2 styles:(CDStruct_506f5052 )arg3 infos:(CDStruct_9d1ebe49 )arg4 inLayout:(id)arg5;

@end

