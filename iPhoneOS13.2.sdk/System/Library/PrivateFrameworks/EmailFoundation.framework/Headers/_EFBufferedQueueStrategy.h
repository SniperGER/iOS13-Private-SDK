//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Dec  5 2019 09:59:31).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <EmailFoundation/EFQueueingStrategy-Protocol.h>

@class NSString;

__attribute__((visibility("hidden")))
@interface _EFBufferedQueueStrategy : NSObject <EFQueueingStrategy>
{
    id /* CDUnknownBlockType */ _batchHandler;
    NSUInteger _capacity;
}

@property(readonly, nonatomic) NSUInteger capacity; // @synthesize capacity=_capacity;
@property(readonly, nonatomic) id /* CDUnknownBlockType */ batchHandler; // @synthesize batchHandler=_batchHandler;
// - (void).cxx_destruct;
- (void)dequeueObject:(id)arg1 buffer:(id)arg2;
- (void)enqueueObject:(id)arg1 replaceIfExists:(BOOL)arg2 buffer:(id)arg3;
@property(readonly, nonatomic) NSString *descriptionType;
- (id)initWithCapacity:(NSUInteger)arg1 batchHandler:(CDUnknownBlockType)arg2;

@end

