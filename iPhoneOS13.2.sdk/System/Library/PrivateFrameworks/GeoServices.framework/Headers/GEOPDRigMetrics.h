//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Dec  5 2019 09:59:31).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <ProtocolBuffer/PBCodable.h>


__attribute__((visibility("hidden")))
@interface GEOPDRigMetrics : PBCodable <NSCopying>
{
//     CDStruct_9f2792e4 _occlusionScores;
}

+ (BOOL)isValid:(id)arg1;
- (void)mergeFrom:(id)arg1;
- (NSUInteger)hash;
- (BOOL)isEqual:(id)arg1;
// - (id)copyWithZone:(_NSZone )arg1;
- (void)copyTo:(id)arg1;
- (void)writeTo:(id)arg1;
- (BOOL)readFrom:(id)arg1;
- (void)readAll:(BOOL)arg1;
- (id)dictionaryRepresentation;
- (id)description;
- (void)setOcclusionScores:(unsigned int )arg1 count:(NSUInteger)arg2;
- (unsigned int)occlusionScoreAtIndex:(NSUInteger)arg1;
- (void)addOcclusionScore:(unsigned int)arg1;
- (void)clearOcclusionScores;
@property(readonly, nonatomic) unsigned int occlusionScores;
@property(readonly, nonatomic) NSUInteger occlusionScoresCount;
- (void)dealloc;

@end

