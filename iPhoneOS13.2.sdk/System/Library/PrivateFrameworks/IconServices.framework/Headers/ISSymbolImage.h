//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Dec  5 2019 09:59:31).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <IconServices/ISImage.h>

__attribute__((visibility("hidden")))
@interface ISSymbolImage : ISImage
{
    CGImageRef _internalCGImage;
    double _internalScale;
    CGSize _internalSize;
    BOOL _flippable;
    double _pointSize;
    NSUInteger _symbolSize;
    long long _symbolWeight;
    double _referencePointSize;
    double _fontMatchingScaleFactor;
    double _baselineOffset;
    double _capHeight;
    CGRect _alignmentRect;
    CGRect _contentBounds;
}

@property(readonly, nonatomic, getter=isFlippable) BOOL flippable; // @synthesize flippable=_flippable;
@property(readonly, nonatomic) CGRect contentBounds; // @synthesize contentBounds=_contentBounds;
@property(readonly, nonatomic) CGRect alignmentRect; // @synthesize alignmentRect=_alignmentRect;
@property(readonly, nonatomic) double capHeight; // @synthesize capHeight=_capHeight;
@property(readonly, nonatomic) double baselineOffset; // @synthesize baselineOffset=_baselineOffset;
@property(readonly, nonatomic) double fontMatchingScaleFactor; // @synthesize fontMatchingScaleFactor=_fontMatchingScaleFactor;
@property(readonly, nonatomic) double referencePointSize; // @synthesize referencePointSize=_referencePointSize;
@property(readonly, nonatomic) long long symbolWeight; // @synthesize symbolWeight=_symbolWeight;
@property(readonly, nonatomic) NSUInteger symbolSize; // @synthesize symbolSize=_symbolSize;
@property(readonly, nonatomic) double pointSize; // @synthesize pointSize=_pointSize;
- (CGSize)size;
- (double)scale;
- (CGImageRef)cgImage;
- (BOOL)placeholder;
- (void)dealloc;
- (id)initWithNamedVectorGlyph:(id)arg1;

@end

