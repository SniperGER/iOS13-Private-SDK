//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <Engram/ENGroupContextObserverMiddleware-Protocol.h>

@class NSString;
@protocol OS_dispatch_queue;

@interface ENGroupContextNotifyingObserver : NSObject <ENGroupContextObserverMiddleware>
{
    NSObject<OS_dispatch_queue> *_queue;
    id /* block */ _onGroupCache;
    id /* block */ _onGroupCreate;
    id /* block */ _onGroupUpdate;
}

@property(copy, nonatomic) id /* block */ onGroupUpdate; // @synthesize onGroupUpdate=_onGroupUpdate;
@property(copy, nonatomic) id /* block */ onGroupCreate; // @synthesize onGroupCreate=_onGroupCreate;
@property(copy, nonatomic) id /* block */ onGroupCache; // @synthesize onGroupCache=_onGroupCache;
@property(retain, nonatomic) NSObject<OS_dispatch_queue> *queue; // @synthesize queue=_queue;
- (void)groupContext:(id)arg1 didUpdateGroup:(id)arg2 withNewGroup:(id)arg3;
- (void)groupContext:(id)arg1 didCacheGroup:(id)arg2;
- (void)groupContext:(id)arg1 didCreateGroup:(id)arg2;
- (id)initWithQueue:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) NSUInteger hash;
@property(readonly) Class superclass;

@end

