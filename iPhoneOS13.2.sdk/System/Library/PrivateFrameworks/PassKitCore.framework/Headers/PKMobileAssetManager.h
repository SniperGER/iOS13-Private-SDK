//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@protocol OS_dispatch_queue;

@interface PKMobileAssetManager : NSObject
{
    NSObject<OS_dispatch_queue> *_queue;
}

+ (id)sharedInstance;
- (id)_downloadOptionsWithUserInitiation:(_Bool)arg1;
- (void)_downloadAsset:(id)arg1 userInitiated:(_Bool)arg2 completion:(id /* block */)arg3;
- (void)_executeQueries:(id)arg1 currentIndex:(unsigned long long)arg2 userInitiated:(_Bool)arg3 sortDescriptors:(id)arg4 completion:(id /* block */)arg5;
- (void)_retrieveAssetWithQueries:(id)arg1 latestCompatibilityVersion:(unsigned long long)arg2 userInitiated:(_Bool)arg3 sortDescriptors:(id)arg4 completionHandler:(id /* block */)arg5;
- (id)_compatibleVersionsWithMinimumVersion:(unsigned long long)arg1 maximumVersion:(unsigned long long)arg2;
- (id)_queriesForCityCodeStationProvider:(id)arg1;
- (id)_queriesForMarketIdentifer:(id)arg1;
- (id)_queriesForEducationVideo;
- (id)sortDescriptorsForCityCode;
- (id)sortDescriptorsForMarkets;
- (void)fetchCityStationProviderAssetForBaseProvider:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)fetchAssetsForMarketWithIdentifier:(id)arg1 userInitiated:(_Bool)arg2 completionHandler:(id /* block */)arg3;
- (id)_sortDescriptorWithPreferredFeatureKey:(id)arg1 isFeaturePreferred:(_Bool)arg2 keyPath:(id)arg3;
- (id)sortDescriptorsForEducationVideoWithPreferredFeatures:(unsigned long long)arg1;
- (void)fetchEducationVideoWithPreferredFeatures:(unsigned long long)arg1 completionHandler:(id /* block */)arg2;
- (void)fetchNeededAssetsForThisDeviceWithPreferredFeatures:(unsigned long long)arg1;
- (id)init;

@end

