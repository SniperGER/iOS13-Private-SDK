//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <AvatarUI/AVTTransition-Protocol.h>

@class NSString;
@protocol AVTTransitionModel, AVTUILogger;

@interface AVTTransition : NSObject <AVTTransition>
{
    _Bool _animated;
    long long _state;
    id <AVTTransitionModel> _model;
    id /* block */ _setupHandler;
    id /* block */ _completionHandler;
    id <AVTUILogger> _logger;
}

@property(readonly, nonatomic) id <AVTUILogger> logger; // @synthesize logger=_logger;
@property(copy, nonatomic) id /* block */ completionHandler; // @synthesize completionHandler=_completionHandler;
@property(copy, nonatomic) id /* block */ setupHandler; // @synthesize setupHandler=_setupHandler;
@property(readonly, nonatomic) id <AVTTransitionModel> model; // @synthesize model=_model;
@property(nonatomic) _Bool animated; // @synthesize animated=_animated;
@property(nonatomic) long long state; // @synthesize state=_state;
@property(readonly, copy) NSString *description;
- (void)performCancellation;
- (void)cancel;
- (void)performTransition;
- (void)start;
- (id)initWithModel:(id)arg1 animated:(_Bool)arg2 setupHandler:(id /* block */)arg3 completionHandler:(id /* block */)arg4 logger:(id)arg5;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly) NSUInteger hash;
@property(readonly) Class superclass;

@end

