//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <TouchML/TMLPointerJSExports-Protocol.h>

@interface TMLPointer : NSObject <TMLPointerJSExports>
{
    id /* block */ _readBlock;
    id /* block */ _writeBlock;
}

+ (id)pointerWithReadBlock:(id /* block */)arg1 writeBlock:(id /* block */)arg2;
@property(copy, nonatomic) id /* block */ writeBlock; // @synthesize writeBlock=_writeBlock;
@property(copy, nonatomic) id /* block */ readBlock; // @synthesize readBlock=_readBlock;
@property(nonatomic) id pointee;

@end

