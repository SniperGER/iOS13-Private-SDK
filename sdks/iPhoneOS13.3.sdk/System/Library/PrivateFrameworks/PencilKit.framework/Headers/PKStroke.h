//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Mar 22 2020 01:47:48).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>


@class NSArray, NSUUID, PKInk, _PKStrokeClipPlane, _PKStrokeData;

@interface PKStroke : NSObject <NSCopying, NSMutableCopying>
{
    struct shared_ptr<PKProtobufUnknownFields> _unknownFields;
    CGRect _cachedBounds;
    PKInk *_ink;
    CDStruct_87ef4b51 _flags;
    NSUUID *_strokeUUID;
    _PKStrokeData *_strokeData;
    NSArray *_substrokes;
    _PKStrokeClipPlane *__clipPlane;
    CDStruct_88b945db _tRange;
    struct _PKStrokeID _version;
    struct _PKStrokeID _sortID;
    struct _PKStrokeID _substrokesVersion;
    CGAffineTransform _transform;
}

+ (id)_testStrokeFromPoints:(struct PKCompressedStrokePoint )arg1 length:(long long)arg2 ink:(id)arg3;
+ (id)copyStrokes:(id)arg1 hidden:(BOOL)arg2;
+ (long long)_asciiBitfieldIndexForX:(long long)arg1 y:(long long)arg2 width:(long long)arg3;
+ (long long)_asciiDimensionForBoundsDimension:(double)arg1;
+ (struct _PKStrokePoint)_decompressStrokePoint:(struct PKCompressedStrokePoint)arg1 withTimestamp:(double)arg2;
+ (struct PKCompressedStrokePoint)_compressStrokePoint:(struct _PKStrokePoint)arg1 withTimestamp:(double)arg2;
@property(retain, nonatomic) _PKStrokeClipPlane *_clipPlane; // @synthesize _clipPlane=__clipPlane;
@property(retain, nonatomic) NSArray *_substrokes; // @synthesize _substrokes;
@property(nonatomic) struct _PKStrokeID _substrokesVersion; // @synthesize _substrokesVersion;
@property(nonatomic) CDStruct_88b945db _tRange; // @synthesize _tRange;
@property(nonatomic, setter=_setTransform:) CGAffineTransform _transform; // @synthesize _transform;
@property(nonatomic) struct _PKStrokeID _sortID; // @synthesize _sortID;
@property(retain, nonatomic, setter=_setStrokeUUID:) NSUUID *_strokeUUID; // @synthesize _strokeUUID;
@property(nonatomic) struct _PKStrokeID _version; // @synthesize _version;
@property(nonatomic) CDStruct_87ef4b51 _flags; // @synthesize _flags;
@property(retain, nonatomic) PKInk *ink; // @synthesize ink=_ink;
- (id).cxx_construct;
// - (void).cxx_destruct;
- (id)_dataInUnknownFields;
- (void)_addTestDataToUnknownFields;
@property(retain, nonatomic) _PKStrokeData *_strokeData; // @synthesize _strokeData;
- (unique_ptr_5ffa53b3)_compressedStrokePoints;
- (id)mutableCopyWithZone:(_NSZone )arg1;
- (double)endTimestamp;
- (double)startTimestamp;
- (BOOL)_containsSamePointsAsStroke:(id)arg1;
- (double)_timestamp;
- (double)_timestampAtIndex:(NSUInteger)arg1;
- (double)timestampAtIndex:(NSUInteger)arg1;
- (CGPoint)locationAtIndex:(NSUInteger)arg1;
- (void)enumeratePointsWithTimestep:(double)arg1 usingBlock:(id /* CDUnknownBlockType */)arg2;
- (void)enumeratePointsWithDistanceStep:(double)arg1 usingBlock:(id /* CDUnknownBlockType */)arg2;
- (CGPoint)_locationAtIndex:(NSUInteger)arg1 applyStrokeTransform:(BOOL)arg2;
- (CGPoint)_locationAtIndex:(NSUInteger)arg1;
- (id)_substrokeWithRange:(NSRange)arg1;
- (id)substrokeWithRange:(NSRange)arg1;
- (long long)compareToStroke:(id)arg1;
- (id)debugQuickLookObject;
- (BOOL )_newAsciiBitfieldWithWidth:(long long)arg1 height:(long long)arg2;
- (BOOL )_newAsciiBitfield;
- (id)_ascii;
- (id)descriptionAtDepth:(long long)arg1;
- (id)description;
- (void)_appendPointsOfInterestForSelection:(vector_e1abc270 )arg1;
- (void)_invalidateBounds;
@property(readonly, nonatomic) CGRect _untransformedBounds;
@property(nonatomic, setter=_setBounds:) CGRect _bounds;
- (CGRect)bounds;
- (CGRect)_calculateBounds;
- (void)_consumeRenderVertexes:(id /* CDUnknownBlockType */)arg1;
- (long long)_inputType;
@property(readonly, nonatomic) double timestamp;
- (NSUInteger)hash;
- (BOOL)isEqual:(id)arg1;
@property(readonly, nonatomic) unsigned int _randomSeed;
- (id)_ink;
- (void)_setInk:(id)arg1;
- (const struct _PKStrokeDataPoints )_points;
- (NSUInteger)_pointsCount;
- (CGPath )_newPathRepresentation;
- (CGPath )newPathRepresentation;
@property(nonatomic, getter=_isNewCopy, setter=_setIsNewCopy:) BOOL _isNewCopy;
@property(readonly, nonatomic) BOOL _isFullyHidden;
@property(nonatomic, getter=_isHidden, setter=_setHidden:) BOOL _hidden;
@property(readonly, nonatomic) NSUUID *_strokeDataUUID;
- (id)copyForMutationWithSubstrokes:(id)arg1;
- (id)copyForMutation;
// - (id)copyWithZone:(_NSZone )arg1;
- (id)initWithData:(id)arg1 id:(id)arg2 sortID:(struct _PKStrokeID)arg3 timestamp:(double)arg4 flags:(CDStruct_87ef4b51)arg5 version:(struct _PKStrokeID)arg6 ink:(id)arg7 transform:(CGAffineTransform)arg8 tRange:(CDStruct_88b945db)arg9 substrokes:(id)arg10 substrokesVersion:(struct _PKStrokeID)arg11;
- (id)initWithData:(id)arg1 id:(id)arg2 sortID:(struct _PKStrokeID)arg3 timestamp:(double)arg4 hidden:(BOOL)arg5 version:(struct _PKStrokeID)arg6 ink:(id)arg7 transform:(CGAffineTransform)arg8 tRange:(CDStruct_88b945db)arg9 substrokes:(id)arg10 substrokesVersion:(struct _PKStrokeID)arg11;
- (id)initWithStroke:(id)arg1 hidden:(BOOL)arg2 version:(struct _PKStrokeID)arg3 ink:(id)arg4 transform:(CGAffineTransform)arg5 tRange:(CDStruct_88b945db)arg6;
- (id)initWithStroke:(id)arg1 hidden:(BOOL)arg2 version:(struct _PKStrokeID)arg3 transform:(CGAffineTransform)arg4;
- (id)initWithStroke:(id)arg1 hidden:(BOOL)arg2 version:(struct _PKStrokeID)arg3;
- (id)initWithStroke:(id)arg1;
- (id)_initWithStroke:(id)arg1 strokeData:(id)arg2;
- (id)_initWithPath:(CGPath )arg1 ink:(id)arg2 inputScale:(double)arg3 velocityForDistanceFunction:(id /* CDUnknownBlockType */)arg4;
- (id)_initWithPath:(CGPath )arg1 ink:(id)arg2 inputScale:(double)arg3;
- (id)init;
- (BOOL)_shouldBeClippedAgainstLegacyCanvas;
- (void)generateClipPaths:(vector_acef39cc )arg1 transform:(CGAffineTransform)arg2;
- (id)strokeApplying:(id)arg1;
- (id)deltaTo:(id)arg1;
- (void)saveToArchive:(struct StrokeDelta )arg1;
- (id)dataRepresentation;
- (id)initWithArchive:(const struct StrokeDelta )arg1 error:(id )arg2;
- (id)initWithData:(id)arg1 error:(id )arg2;
- (id)_updateWithParent:(id)arg1;
- (id)_mergeUnparentedWithStroke:(id)arg1;
- (id)mergeArrayOfStrokes:(id)arg1 with:(id)arg2;
- (id)_mergeWithStroke:(id)arg1;
- (id)_updateStroke:(id /* CDUnknownBlockType */)arg1;
- (BOOL)_visitVisibleSubstrokes:(id /* CDUnknownBlockType */)arg1;
- (void)_addVisibleSubstrokesTo:(id)arg1;
- (long long)_visibleSubstrokeCount;
- (id)_sliceWithSlicingBlock:(id /* CDUnknownBlockType */)arg1 replica:(id)arg2;
- (BOOL)intersectsClosedStroke:(id)arg1;
- (BOOL)intersectsLineFrom:(CGPoint)arg1 to:(CGPoint)arg2 minThreshold:(double)arg3;
- (vector_acef39cc )newOutlinePaths;
- (void)debugRender:(CGContext )arg1;
- (void)interpolatePointDataWithStep:(double)arg1 from:(double)arg2 to:(double)arg3 usingBlock:(id /* CDUnknownBlockType */)arg4;
- (void)interpolatePointDataWithStep:(double)arg1 usingBlock:(id /* CDUnknownBlockType */)arg2;
- (unsigned int)saveToV1Archive:(struct Stroke )arg1 sortedUUIDs:(id)arg2 inks:(id)arg3 withPathData:(BOOL)arg4 transient:(BOOL)arg5;
- (struct _PKStrokePoint)readPointFromLegacyArchive:(const struct Point )arg1 deltaFrom:(const struct _PKStrokePoint )arg2;
- (CGPoint)readPointFromArchive:(const struct Point )arg1;
- (void)saveV1StrokeID:(const struct _PKStrokeID )arg1 toArchive:(struct StrokeID )arg2 withSortedUUIDs:(id)arg3;
- (struct _PKStrokeID)readV1StrokeIDFromArchive:(const struct StrokeID )arg1 withSortedUUIDs:(id)arg2;
- (void)saveStrokeID:(const struct _PKStrokeID )arg1 toArchive:(struct StrokeID )arg2 withSortedUUIDs:(id)arg3;
- (struct _PKStrokeID)readStrokeIDFromArchive:(const struct StrokeID )arg1 withSortedUUIDs:(id)arg2;
- (id)initWithLegacyArchive:(const struct Command )arg1 sortedUUIDs:(id)arg2;
- (id)initWithV1Archive:(const struct Stroke )arg1 sortedUUIDs:(id)arg2 inks:(id)arg3;
- (unsigned int)saveToArchive:(struct Stroke )arg1 sortedUUIDs:(id)arg2 inks:(id)arg3 withPathData:(BOOL)arg4 parent:(id)arg5 transient:(BOOL)arg6;
- (unsigned int)saveToArchive:(struct Stroke )arg1 sortedUUIDs:(id)arg2 inks:(id)arg3 withPathData:(BOOL)arg4 transient:(BOOL)arg5;
- (void)saveInksTo:(id)arg1;
- (void)saveUUIDsTo:(id)arg1;
- (id)initWithArchive:(const struct Stroke )arg1 sortedUUIDs:(id)arg2 inks:(id)arg3 parent:(id)arg4 isHidden:(BOOL)arg5;
- (id)initWithArchive:(const struct Stroke )arg1 sortedUUIDs:(id)arg2 inks:(id)arg3;

@end

